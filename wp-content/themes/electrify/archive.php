<?php get_header();
    ...
    if (is_category()) {
        ...
        $arc_title = __("Posts Categorized:", "electrify") . ' ' . single_cat_title( $prefix = '', $display = false );
    
    }
    elseif (is_tag()) { 

        $arc_title = __("Posts Tagged:", "electrify") . ' ' . single_tag_title( $prefix = '', $display = false );
    ...
    elseif (is_author()) { 
        ...
        $arc_title = __("Posts By:", "electrify") . ' ' . get_the_author_meta('display_name', $author_id);

    }
    elseif (is_day()) { 

        $arc_title = __("Daily Archives:", "electrify") . ' ' . get_the_time('l, F j, Y');

    }
    elseif (is_month()) {  

        $arc_title = __("Monthly Archives:", "electrify") . ' ' . get_the_time('F Y');

    }
    elseif (is_year()) {  

        $arc_title = __("Posts Categorized:", "electrify") . ' ' . get_the_time('Y');
...
<?php if ($a_styles == 'grid' || $a_styles == 'masonry' || $a_styles == 'masonry_with_sb' || $a_styles == 'grid_with_sb'){
    ...
    if($sb == 'yes' && $select_sidebar == 'left'){
        ...
        if ( is_active_sidebar( $sidebar_name ) ) {
            ...
            echo '<p>'.__("Please activate some Widgets Or disable it from theme option", "electrify").'</p>';
        }
        echo '</div>';
    }
                ...
                if (have_posts()) : while (have_posts()) : the_post();
                    ...
                    if($a_meta){
                        $meta = '<div class="byline vcard clearfix">
                                    <p class="pull-left category">
                                        '
                                        .sprintf( __( '<span class="tag"><i class="pixicon pixicon-eleganticons-217"></i> %1$s </span>', 'electrify' ),get_the_category_list(', ')).'<span class="pix-blog-comments"><i class="pixicon pixicon-eleganticons-196"></i>'. get_comments_popup_link(__('No Comments','electrify'), __('1 Comment','electrify'), __('% Comments','electrify'), __('comments-link','electrify'), '') .'</span>
                    ...
                    <div class="blog-container <?php echo $content_class; ?>">
                        ...
                        <div class="author">    
                            ...
                            <div>
                                <?php printf( __( '<p class="author-name"> %1$s</p><br><p class="date"> %2$s</p>', 'electrify' ),pixel8es_get_the_author_posts_link(),get_the_date ('M j, Y')); ?> 
                            </div>
...
<?php if ( function_exists( 'pixel8es_page_navi' ) ) { ?>
        ...
        <nav class="wp-prev-next ">
                <ul class="clearfix">
                    <li class="prev-link"><?php next_posts_link( __( '&laquo; Older Entries', 'electrify' )) ?></li>
                    <li class="next-link"><?php previous_posts_link( __( 'Newer Entries &raquo;', 'electrify' )) ?></li>
                </ul>
        </nav>
<?php } ?> 
