...
function pixel8es_scripts_and_styles() {
	...
	if (!is_admin()) {
		...
		wp_register_style( 'pix-fonts', get_template_directory_uri() . '/library/css/pix-icons.css', array(), '4.3', 'all' );
		...
		// register bootstrap stylesheet
	        ...
	        wp_register_style( 'electrify-stylesheet', get_template_directory_uri() . '/library/css/style.min.css', array(), '4.3', 'all' );
	        ...
	        if($mobile_responsive == 1){
	        	wp_register_style( 'electrify-responsive-stylesheet', get_template_directory_uri() . '/library/css/responsive.css', array(), '4.3', 'all' );
	        }else{
	        	wp_register_style( 'electrify-responsive-stylesheet', get_template_directory_uri() . '/library/css/non-responsive.css', array(), '4.3', 'all' );
	        }

	        // register main stylesheet
	        wp_register_style( 'electrify-animate-stylesheet', get_template_directory_uri() . '/library/css/animate.min.css', array(), '4.3', 'all' );

	         //woo stylesheet
	        wp_register_style( 'electrify-woo-stylesheet', get_template_directory_uri() . '/library/css/woo.css', array('electrify-plugins-stylesheet'), '4.3', 'all' );

	        // register plugins stylesheet
	        wp_register_style( 'electrify-plugins-stylesheet', get_template_directory_uri() . '/library/css/plugins.css', array('electrify-stylesheet'), '4.3', 'all' );
		
	        $theme_color = isset($smof_data['theme_color'])? $smof_data['theme_color'] : 'default';

	        // register Color CSS
	        if($theme_color != ''){
	        	wp_register_style( 'electrify-color-stylesheet', get_template_directory_uri() . '/library/css/color-css/'.$theme_color.'.css', array('electrify-plugins-stylesheet'), '4.3', 'all' );
	        }

	        // register custom stylesheet
		    wp_register_style( 'electrify-custom-css', get_template_directory_uri() . '/library/css/custom_styles.css', array('electrify-plugins-stylesheet'), '4.3', 'all' );
		...
		$api_key = isset($smof_data['api_key']) ? $smof_data['api_key'] : '';

		wp_register_script( 'gmap', $protocol.'://maps.googleapis.com/maps/api/js?key='. $api_key .'&sensor=false', array(), '4.3', false );
		...
		if (class_exists('WPBakeryVisualComposerAbstract')) {
			...
			wp_register_script( 'pixel8es-plugins-js', get_template_directory_uri() . '/library/js/plugins.js', array( 'jquery' ), '4.3', $load_script_in_footer );
		}

		wp_register_script( 'pixel8es-js', get_template_directory_uri() . '/library/js/scripts.js', array( 'jquery', 'pixel8es-plugins-js' ), '4.3', true );

		// wp_register_script( 'pixel8es-js', get_template_directory_uri() . '/library/js/scripts.min.js', array( 'jquery', 'pixel8es-plugins-js' ), '4.3', true );
		...
		*/
		...
		wp_localize_script( 'pixel8es-plugins-js', 'electrifyAjaxForm',
			array(
				'nameError' => __('Please enter your name!', 'electrify'),
				'nameLenError' => __('Your name needs to be at least {0} characters', 'electrify'),
				'emailError' => __('Please enter a valid email address.', 'electrify'),
				'emailLenError' => __('Your email address must be in the format of name@domain.com', 'electrify'),
				'subjectError' => __('You need to enter a subject!', 'electrify'),
				'subjectLenError' => __('Subject needs to be at least {0} characters', 'electrify'),                           
				'messageError' => __('You need to enter a message!', 'electrify'),
				'messageLenError' => __('Message needs to be at least {0} characters', 'electrify')
				)
			);
...
function pixel8es_theme_support() {
	...
	register_nav_menus(
		array(
			'main-nav' => __( 'The Main Menu', 'electrify' ),   // main nav in header
			'top-head-nav' => __( 'Top Header Nav', 'electrify' ),
			'left-nav' => __( 'Left Side Nav', 'electrify' ),
			'right-nav' => __( 'Right Side Nav', 'electrify' ),
			'top-nav' => __( 'Top Sub Nav', 'electrify' ),
			'404-nav' => __( '404 Nav', 'electrify' )
		)
	);
} /* end pixel8es theme support */

function pixel8es_add_editor_styles() {
    add_editor_style( 'custom-editor-style.css' );
}
add_action( 'admin_init', 'pixel8es_add_editor_styles' );
...
function pixel8es_nav_fallback() {
	echo '<p><a href="'.home_url().'/wp-admin/nav-menus.php">'.__('Click Here to create/add Menu from Admin Page','electrify').'</a></p>';
}
// this is the fallback for header menu
function pixel8es_top_nav_fallback() {
	echo '<a class="pull-left" href="'.home_url().'/wp-admin/nav-menus.php">'.__('add Menu from Admin Page','electrify').'</a>';
}
...
function pixel8es_related_posts() {
	...
	if($tags) {
		...
		if($related_posts) {
			...
			<?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'electrify' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_query();
	echo '</ul>';
...
function pixel8es_page_navi($before = '', $after = '') {
	...
	if ($start_page >= 2 && $pages_to_show < $max_page) {
		$first_page_text = __( "First", 'electrify' );
	...
	$output .= '</li>';
	if ($end_page < $max_page) {
		$last_page_text = __( "Last", 'electrify' );
...
function pixel8es_get_the_author_posts_link() {
	...
	$link = sprintf(
		...
		esc_attr( sprintf( __( 'Posts by %s','electrify' ), get_the_author() ) ), // No further l10n needed, core will take care of this one
		get_the_author()
	);
	return $link;
}
