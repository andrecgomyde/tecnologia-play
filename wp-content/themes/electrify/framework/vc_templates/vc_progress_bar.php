...
if ($custombgcolor!='') { 
    $custombgcolor = ' style="background-color: '.$custombgcolor.';"'; $bgcolor="";
}
...
$output = '<div class="'.$css_class.' pix-progress-bar '.$style.'">';
...
foreach($graph_lines_data as $line) {
   ...
   // $output .= '<small class="vc_label">'. $line['label'] . $unit .'</small>';
    ...
    $output .= '<div class="vc_bar'.$bar_options.'" data-percentage-value="'.($percentage_value).'" data-value="'.$line['value'].'"'.$line['bgcolor'].'>
	
				<small class="vc_label">'. $line['label'] . $unit .'</small>
