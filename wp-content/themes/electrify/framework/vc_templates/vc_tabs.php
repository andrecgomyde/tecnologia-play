...
foreach ( $tab_titles as $tab ) {
    ...
    //var_dump($tab_icon[1][0]);
    if(isset($tab_matches[1][0])) {
        $tabs_nav .= '<li>';
        $icons = '';
        if( $tab_icon[1][0] != '' ){
            $icons ='<i class="pixicon '.$tab_icon[1][0].'"></i>';
        }
        //var_dump($icons);
...
if ( 'vc_tour' == $this->shortcode) {
    $output .= "\n\t\t\t" . '<div class="wpb_tour_next_prev_nav vc_clearfix"> <span class="wpb_prev_slide"><a href="#prev" title="'.__('Previous slide', 'electrify').'">'.__('Previous slide', 'electrify').'</a></span> <span class="wpb_next_slide"><a href="#next" title="'.__('Next slide', 'electrify').'">'.__('Next slide', 'electrify').'</a></span></div>';
}
