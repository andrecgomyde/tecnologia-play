...
class WPTuts_Flickr extends WP_Widget {
	...
	function  __construct() {
		$widget_ops = array( 'classname' => 'wptuts-flickr', 'description' => __('Display your Flickr photo gallery', 'electrify' ) );
		parent:: __construct ('wptuts-flickr-widget', __('Pixel8es:: Flickr', 'electrify' ), $widget_ops);
	}
...
?>
	...
	function form( $instance ) {
		...
		<!-- widget title -->
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'electrify' ); ?></label>
	 ...
	 <p>
        ...
        ?>
		<label for="<?php echo $this->get_field_id('flickrid'); ?>"><?php _e('Your Flickr User ID:', 'electrify' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('flickrid'); ?>" type="text" name="<?php echo $this->get_field_name('flickrid'); ?>" value="<?php echo isset($instance['flickrid']) ? esc_attr($instance['flickrid']) : '' ; ?>" />
 		<small>Don't know your ID? Head on over to <a href="<?php echo $protocol; ?>idgettr.com">idgettr</a> to find it.</small>
	 </p>

        <p><label for="<?php echo $this->get_field_id('flickrCount');?>">
        <?php _e('Flickr Image Count (Max 20):', 'electrify' ); ?>
