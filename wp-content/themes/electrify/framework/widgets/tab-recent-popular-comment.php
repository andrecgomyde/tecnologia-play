...
class Pix_Tab_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_options = array('classname' => 'tab-widget', 'description' => __('Display Tab Widget','electrify'));
		parent::__construct('pix_tab_widget',__('Pixel8es:: Tab Widget','electrify'),$widget_options);
	}
	...
	function widget($args, $instance) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Recent Posts','electrify' );
		$title = apply_filters( 'widget_title', $title, $instance, $this->id_base );


		$tab_content = array('popular' => __( 'Popular Post','electrify' ),'recent' => __( 'Recent Post','electrify' ), 'comment' => __( 'Recent Comments','electrify' ));
			...
			if($key == 'comment'){
				$recent_comment = '<ul id="recentcomments">';
					...
					foreach ( (array) $comments as $comment) {
						$recent_comment .=  '<li class="recentcomments">' . /* translators: comments widget: 1: comment author, 2: post link */ sprintf(__('%1$s on %2$s', 'electrify'), get_comment_author_link($comment->comment_ID), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
					}
...
?>
		<h4><?php _e( 'Popular Posts:','electrify' ) ?></h4>
		<p><label for="<?php echo $this->get_field_id( 'p_number' ); ?>"><?php _e( 'Number of Popular Posts:', 'electrify' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id( 'p_show_date' ); ?>"><?php _e( 'Display post date?' , 'electrify'); ?></label></p>
		...
		<label for="<?php echo $this->get_field_id( 'p_show_image' ); ?>"><?php _e( 'Display post image?', 'electrify' ); ?></label></p>


		<h4><?php _e( 'Recent Posts:','electrify' ) ?></h4>
		<p><label for="<?php echo $this->get_field_id( 'r_number' ); ?>"><?php _e( 'Number of Popular Posts:', 'electrify' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id( 'r_show_date' ); ?>"><?php _e( 'Display post date?', 'electrify' ); ?></label></p>
		...
		<label for="<?php echo $this->get_field_id( 'r_show_image' ); ?>"><?php _e( 'Display post image?', 'electrify' ); ?></label></p>

		<h4><?php _e( 'Recent Comments:','electrify' ) ?></h4>
		<p><label for="<?php echo $this->get_field_id( 'c_number' ); ?>"><?php _e( 'Number of Popular Posts:', 'electrify' ); ?></label>
