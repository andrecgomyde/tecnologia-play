...
function pix_weather_logic( $atts )
{
	...
	$units_display		= $units == "metric" ? __('C', 'electrify') : __('F', 'electrify');
	...
	if($show_stats)
	{
		$speed_text = ($units == "metric") ? __('km/h', 'electrify') : __('mph', 'electrify');
		...
		$rtn .= "
				...
				<div class=\"pix-weather-todays-stats\">
					...
					<div class=\"awe_humidty\">" . __('humidity:', 'electrify') . " {$today->main->humidity}% </div>
					<div class=\"awe_wind\">" . __('wind:', 'electrify') . " {$today->wind->speed}" . $speed_text . " {$wind_direction}</div>
					<div class=\"awe_highlow\"> "  .__('H', 'electrify') . " {$today_high} &bull; " . __('L', 'electrify') . " {$today_low} </div>	
	...
	if($days_to_show != "hide")
	{
		...
		foreach( (array) $forecast->list as $forecast )
		{
			...
			$days_of_week = array( __('Sun' ,'electrify'), __('Mon' ,'electrify'), __('Tue' ,'electrify'), __('Wed' ,'electrify'), __('Thu' ,'electrify'), __('Fri' ,'electrify'), __('Sat' ,'electrify') );
	...
	{
		$show_link_text = apply_filters('pix_weather_extended_forecast_text' , __('extended forecast', 'electrify'));
...
{
	if(!$msg) $msg = __('No weather information available', 'electrify');
...
{
	function __construct() { parent::__construct(false, $name = __('Pixel8es:: Weather Widget','electrify')); }
	...
	?>
        <p>
          <label for="<?php echo $this->get_field_id('location'); ?>">
          	<?php _e('Location:', 'electrify'); ?><br />
          	<small><?php _e('(i.e: London,UK or New York City,NY)', 'electrify'); ?></small>
          </label> 
        ...
        <p>
          <label for="<?php echo $this->get_field_id('override_title'); ?>"><?php _e('Override Title:', 'electrify'); ?></label> 
        ...
        <p>
          <label for="<?php echo $this->get_field_id('units'); ?>"><?php _e('Units:', 'electrify'); ?></label>  &nbsp;
		...
		<p>
          <label for="<?php echo $this->get_field_id('size'); ?>"><?php _e('Size:', 'electrify'); ?></label> 
		...
		<p>
          <label for="<?php echo $this->get_field_id('forecast_days'); ?>"><?php _e('Forecast:', 'electrify'); ?></label> 
        ...
        <p>
          <label for="<?php echo $this->get_field_id('hide_stats'); ?>"><?php _e('Hide Stats:', 'electrify'); ?></label>  &nbsp;
        ...
        <p>
          <label for="<?php echo $this->get_field_id('show_link'); ?>"><?php _e('Link to OpenWeatherMap:', 'electrify'); ?></label>  &nbsp;
        ...
        <p>
          <label for="<?php echo $this->get_field_id('widget_title'); ?>"><?php _e('Widget Title: (optional)', 'electrify'); ?></label> 
