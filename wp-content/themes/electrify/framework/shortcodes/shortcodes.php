...
class Pixel8esShortcodes {
	...
	function pix_contact($atts, $content = null, $code){
		...
		$contact_button_text = isset($smof_data['contact_button_text']) ? $smof_data['contact_button_text'] : __('SEND MAIL','electrify');
		
		if(empty($contact_button_text)){
			$contact_button_text = __('Send Mail','electrify');
		}
	   ...
	   $output .= '<div class="contactForm'. esc_attr($animate_class) .'"'.$slideTransition.''.$slideDuration.''.$slideDelay.'>	           
				   ...
				   <div class="row">
					   ...
					   <p class="textarea clear">
						   <label for="message">'.__('Message:','electrify').'  <span class="color">*</span></label>
		...
		if ($the_query->have_posts()) : while ($the_query->have_posts()) : $the_query->the_post();
			...
			if ('open' == $the_query->post->comment_status){
				...
				if($display_comments == 'yes'){
					$meta .= '<span class="pix-blog-comments">'. get_comments_popup_link(__('No Comments','electrify'), __('1 Comment','electrify'), __('% Comments','electrify'), __('comments-link','electrify'), '') .'</span>';
				}
			}
