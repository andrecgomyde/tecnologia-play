...
//Menu Function
electrify_require_file (THEME_FUNCTIONS . '/pix-menu-extend/pix-menu-extend.php');
electrify_require_file (THEME_FUNCTIONS . '/PixIterator/Iterator.php');
electrify_require_file (THEME_FUNCTIONS . '/PixIterator/Icon.php');
electrify_require_file (THEME_FUNCTIONS . '/pix-like-me/like-me.php');
...
function pix_import_demo(){
	...
	electrify_require_file (THEME_FUNCTIONS . '/pix-importer/pix-importer.php');
	...
	?>      
			...
			<div class="cart-trigger">
				<div class="pix-cart-contents-con">
					<span class="pix-no-items"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'electrify'), $woocommerce->cart->cart_contents_count);?></span> 
					<span class="pix-woo-price"><?php echo $woocommerce->cart->get_cart_total(); ?></span>
					
					<a class="pix-cart-contents" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'electrify'); ?>"><span class="pixicon-basket pix-cart-icon"></span><span class="pix-item-icon"><?php echo $woocommerce->cart->cart_contents_count; ?></span></a>
				</div>
...
$GLOBALS['pix_icons_class_html'] = $pix_icons_class_html;
...
//if ($showCurrent == 1) echo '<li>' . $delimiter . ' ' . $before . ucwords(get_the_title()) . $after . '</li>';
	...
	if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
			echo __('Page', 'electrify') . ' ' . get_query_var('paged');
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
}
...
function electrify_admin_scripts($hook_suffix) {	
	...
	if( 'post.php' == $hook_suffix || 'post-new.php' == $hook_suffix ) {
		//styles
		wp_enqueue_style( 'metabox-css', THEME_CUSTOM_METABOXES_URI. '/css/metabox.css', array(), '1.0' );
...
function electrify_formsubmit(){
	...
	if(empty($contact_error)){
		$contact_error = __('Please check if you have filled all the fields with valid information and try again. Thank you.','electrify');
	}
...
function make_twitter($tweet) {
	...
	$tweet = preg_replace('/(^|\s)@(\w+)/','\1@<a href="'. $protocol .'www.twitter.com/\2">\2</a>',$tweet);
	return preg_replace('/(^|\s)#(\w+)/','\1#<a href="'. $protocol .'search.twitter.com/search?q=%23\2">\2</a>',$tweet);
	return $tweet;
}
...
function get_comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	if ( false === $zero ) $zero = __( 'No Comments', 'electrify' );
	if ( false === $one ) $one = __( '1 Comment', 'electrify' );
	if ( false === $more ) $more = __( '% Comments', 'electrify' );
	if ( false === $none ) $none = __( 'Comments Off', 'electrify' );
	...
	if ( post_password_required() ) {
		$str = __('Enter your password to view comments.', 'electrify');
		return $str;
	}
...
$str .= ' title="' . esc_attr( sprintf( __('Comment on %s', 'electrify'), $title ) ) . '">';
...
function get_comments_number_str( $zero = false, $one = false, $more = false, $deprecated = '' ) {
	...
	if ( $number > 1 )
		$output = str_replace('%', number_format_i18n($number), ( false === $more ) ? __('% Comments', 'electrify') : $more);
	elseif ( $number == 0 )
		$output = ( false === $zero ) ? __('No Comments', 'electrify') : $zero;
else // must be one
$output = ( false === $one ) ? __('1 Comment', 'electrify') : $one;
...
if (class_exists('Woocommerce')) {
	...
	function woocommerce_header_add_to_cart_fragment( $fragments ) {
		...
		?>
		<div class="pix-cart-contents-con">
			<span class="pix-no-items"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'electrify'), $woocommerce->cart->cart_contents_count);?></span> <span class="pix-woo-price"><?php echo $woocommerce->cart->get_cart_total(); ?></span>
...
function pix_languages_list(){
	if(class_exists('SitePress')){
		...
		if($show_lang_sel){
			...
			if(1 < $lang_count){
				$trans_status = __('translated', 'electrify');			
			}else{
				$trans_status = __('not-translated', 'electrify');
			}
				...
				if($lang_display_style == 'dropdown'){
						...
						if($lang_display_style == 'dropdown'){
							...
							if(1 < $lang_count){	
								...
								echo '<span class="already-liked">'. __('Not Translated','electrify') .'</span></div>';
							}
						}
