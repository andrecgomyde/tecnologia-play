...
class Pix_Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu  {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( __( '%s (Invalid)', 'electrify' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( __('%s (Pending)', 'electrify'), $item->title );
		}
		...
		<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					<span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo $submenu_text; ?>><?php _e( 'sub item', 'electrify' ); ?></span></span>
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e( 'Move up', 'electrify' ); ?>">&#8593;</abbr></a>
							|
							<a href="<?php
								echo wp_nonce_url(
									add_query_arg(
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e( 'Move down', 'electrify' ); ?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php esc_attr_e( 'Edit Menu Item', 'electrify' ); ?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php _e( 'Edit Menu Item', 'electrify' ); ?></a>
					</span>
				</dt>
			</dl>
			...
			<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo $item_id; ?>">
						<?php _e( 'Navigation Label', 'electrify' ); ?><br />
						<input type="text" id="edit-menu-item-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
						<?php _e( 'Title Attribute', 'electrify' ); ?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo $item_id; ?>">
						...
						<?php _e( 'Open link in a new window/tab', 'electrify' ); ?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
						<?php _e( 'CSS Classes (optional)', 'electrify' ); ?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
						<?php _e( 'Link Relationship (XFN)', 'electrify' ); ?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo $item_id; ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo $item_id; ?>">
						<?php _e( 'Description', 'electrify' ); ?><br />
						<textarea id="edit-menu-item-description-<?php echo $item_id; ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo $item_id; ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
						<span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.', 'electrify'); ?></span>
					</label>
				</p>
	            ...
	            <p class="pix-field-custom description description-wide"><br class="clearboth"> 
	                <label for="edit-menu-item-subtitle-<?php echo $item_id; ?>">
	                    <?php _e( 'Insert Menu Icon', 'electrify' ); ?><br />
	                    ...
	                    <a href="#" class="button pix-insert-menu-icon"><i class="fa fa-arrow-circle-o-down"></i> <?php _e('Insert Icon', 'electrify'); ?></a>
	                    <a href="#" class="button pix-remove-menu-icon hidden"><i class="fa fa-times"></i> <?php _e('Remove Icon', 'electrify'); ?></a>
	                </label>
	            </p>
				<?php //Mega Menu ?>
	            <p class="pix-field-custom-megamenu description description-wide">
	                <label for="edit-menu-item-megamenu-<?php echo $item_id; ?>">
	                    <?php _e( 'Mega Menu', 'electrify'); ?><br />
						<span class="switch-light switch-candy" onclick="">
							...
							<span><span><?php _e('No','electrify'); ?></span><span><?php _e('Yes','electrify'); ?></span></span><a></a>
						</span>
	                </label>
	            </p>
	            ...
	            <?php //Mega Menu Columns ?>
		            <div class="pix-field-custom-megamenucol description description-wide">
		                <label>
		                    <?php _e( 'Mega Menu Columns', 'electrify'); ?><br />
		                </label>
						<?php 
						...
						<div class="switch-toggle switch-5 switch-candy" onclick="">
							...
							<label for="edit-menu-item-megamenucol2-<?php echo $item_id; ?>" onclick=""><?php _e('2','electrify'); ?></label>
							...
							<label for="edit-menu-item-megamenucol3-<?php echo $item_id; ?>" onclick=""><?php _e('3','electrify'); ?></label>
							...
							<label for="edit-menu-item-megamenucol4-<?php echo $item_id; ?>" onclick=""><?php _e('4','electrify'); ?></label>
							...
							<label for="edit-menu-item-megamenucol5-<?php echo $item_id; ?>" onclick=""><?php _e('5','electrify'); ?></label>
							...
							<label for="edit-menu-item-megamenucol6-<?php echo $item_id; ?>" onclick=""><?php _e('6','electrify'); ?></label>
							<a></a>
						</div>
		            ...
		            <p class="pix-field-custom-megamenupos description description-wide">
		                <label for="edit-menu-item-megamenupos-<?php echo $item_id; ?>">
		                    <?php _e( 'Mega Menu Position', 'electrify'); ?><br />
							<span class="switch-light switch-candy" onclick="">
								...
								<span><span><?php _e('Left','electrify'); ?></span><span><?php _e('Right','electrify'); ?></span></span><a></a>
							</span>
		                </label>
		            </p>
	            ...
	            <p class="pix-field-custom-megamenutitle description description-wide">
	                <label for="edit-menu-item-megamenutitle-<?php echo $item_id; ?>">
	                    <?php _e( 'Hide Title (only hide if mega menu enabled)', 'electrify'); ?><br />
						<span class="switch-light switch-candy" onclick="">
							...
							<span><span><?php _e('Show','electrify'); ?></span><span><?php _e('Hide','electrify'); ?></span></span><a></a>
						</span>
	                </label>
	            </p>
	            ...
	            <div class="pix-field-custom-megamenubgimg description description-wide">
	                <label for="edit-menu-item-megamenubgimg-<?php echo $item_id; ?>">
	                    <?php _e( 'Insert Background Image', 'electrify'); ?><br />
	                </label>	
				...
				<p class="field-move hide-if-no-js description description-wide">
					<label>
						<span><?php _e( 'Move', 'electrify' ); ?></span>
						<a href="#" class="menus-move-up"><?php _e( 'Up one', 'electrify' ); ?></a>
						<a href="#" class="menus-move-down"><?php _e( 'Down one', 'electrify' ); ?></a>
						<a href="#" class="menus-move-left"></a>
						<a href="#" class="menus-move-right"></a>
						<a href="#" class="menus-move-top"><?php _e( 'To the top', 'electrify' ); ?></a>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( __('Original: %s','electrify'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
					...
					); ?>"><?php _e( 'Remove', 'electrify' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo $item_id; ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
						?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel', 'electrify'); ?></a>
				</div>
