	...
	class Pixelentity_Themes_Updater {
		...
		public function check($updates) {
			...
			if (!class_exists("Envato_Protected_API")) {
				electrify_require_file( THEME_FRAMEWORK ."/plugins/pixelentity-themes-updater/class-envato-protected-api.php");
			}
			...
			foreach ($purchased as $theme) {
				if (isset($filtered[$theme]) && is_object($filtered[$theme]) && isset($filtered[$theme->theme_name])) {
