...
class sidebar_generator {
	...
	public static function admin_print_scripts(){
		...
		?>
			<script>
				...
				{
					if(sidebar_name == ''){
						alert ('Sidebar Name not entered!');
						return false;
					}
	...
	public static function add_sidebar(){
		...
		$js = "
			var tbl = document.getElementById('pix_table');
	...
	public static function admin_page(){
		...
		<script>
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("Sidebar Name:",""), result;
				//alert(sidebar_name);
				result = add_sidebar(sidebar_name);

				if(result == true && jQuery('#no-sidebar').length > 0){
					jQuery('#no-sidebar').hide();
				}
		...
		<div class="wrap">
			<h2>Sidebars</h2>
			<br />
			<table class="widefat page" id="pix_table" style="width:600px;">
				<tr>
					<th>Sidebar Name</th>
					<th>CSS class</th>
					<th>Remove</th>
	...
	*/
	public static function save_form($post_id){
		if(isset($_POST['pix_edit'])){
		$is_saving = $_POST['pix_edit'];
		if(!empty($is_saving)){
			delete_post_meta($post_id, 'pix_selected_sidebar');
			delete_post_meta($post_id, 'pix_selected_sidebar_replacement');
			add_post_meta($post_id, 'pix_selected_sidebar', $_POST['sidebar_generator']);
			add_post_meta($post_id, 'pix_selected_sidebar_replacement', $_POST['sidebar_generator_replacement']);
		}
		}
	}
	...
	*/
	public static function get_sidebar($name="0"){
		...
		$selected_sidebar = get_post_meta($post->ID, 'pix_selected_sidebar', true);
		$new_sidebar = get_post_meta($post->ID, 'pix_page_options', true);
		if(!isset($new_sidebar) && !empty($new_sidebar)){
			$selected_sidebar_replacement = array($new_sidebar['selected_sidebar_replacement']);
		}else{
			$selected_sidebar_replacement = array('0');
		}
		
		/*if( !empty($selected_sidebar) )
			extract($selected_sidebar[0]);*/
	...
	*/
	public static function update_sidebars($sidebar_array){
		$sidebars = update_option('pix_sidebars',$sidebar_array);
	}	
	...
	*/
	public static function get_sidebars(){
		$sidebars = get_option('pix_sidebars');
		return $sidebars;
	}
